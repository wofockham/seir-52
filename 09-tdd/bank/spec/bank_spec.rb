...
describe Bank do
    ...
    describe '#withdraw' do
        ...
        it 'ignores withdrawals that exceed the balance' do
            bank.create_account 'New Carter', 1
            bank.withdraw 'New Carter', 100_000_000
            expect(bank.balance('New Carter')).to eq 1
        end
    end

    describe '#balance' do
        it 'returns the balance for some particular account' do
            bank.create_account 'Xavier', 5001
            bal = bank.balance 'Xavier'
            expect(bal).to eq 5001
        end
    end
end